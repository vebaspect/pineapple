...
namespace Pineapple.Core.Handler
{
    public class CreateComponentVersionCommandHandler : RequestHandler<CreateComponentVersionCommand, Task<Guid>>, ICommandHandler
    {
        ...
        protected override async Task<Guid> Handle(CreateComponentVersionCommand request)
        {
            ...
            var componentVersion = Domain.Entities.ComponentVersion.Create(componentVersionId, request.ReleaseDate, request.Major, request.Minor, request.Patch, request.Suffix, request.Description, request.IsImportant, request.ComponentId);
